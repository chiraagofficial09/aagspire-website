...
export function CustomSelect<T extends string | number = string>({
  ...
  return (
    ...
    >
      ...
      {/* Portal-rendered Dropdown Menu — Completely immune to table overflow:hidden or overflow:auto */}
      {isOpen &&
        createPortal(
          <div
            ref={menuRef}
            ...
            className={`rounded-2xl bg-[#0c0d12] border border-white/[0.12] p-1.5 shadow-[0_20px_60px_rgba(0,0,0,0.98)] overflow-y-auto custom-scrollbar select-none animate-in fade-in duration-100 ${menuClassName}`}
          >
            {options.map((opt) => {
